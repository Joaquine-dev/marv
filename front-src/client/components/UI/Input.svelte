...
<label class="flex flex-wrap items-center rounded border border-border {color} {rootClass}">
  {#if label}
  ...
  <div class="flex-auto">
    <input
      ...
      class="{inputClass} w-full bg-transparent text-text-primary border-0 focus:outline-none {inputHidden}"
    />
  </div>
</label>
