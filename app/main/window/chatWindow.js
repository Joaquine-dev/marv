...
function loadChat(win, channel) {
  if (!channel || currentChannel === channel) return;
  win.loadURL(`https://www.twitch.tv/embed/${channel}/chat?parent=localhost&darkpopout`);
...
module.exports = function chatWindow({ channel, showOnLoad = true } = {}) {
  ...
  win.webContents.on("did-finish-load", async () => {
    ...
    // Auto-accept Twitch cookies
    win.webContents.executeJavaScript(`
      (function acceptCookies() {
        const acceptBtn = document.querySelector('[data-a-target="consent-banner-accept"]');
        if (acceptBtn) {
          acceptBtn.click();
